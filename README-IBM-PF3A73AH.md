...
This project requires **Python 3.11+**. Verify with:
...
> This is the fastest path to get the app running on a Windows machine. Run all commands in **PowerShell**.

### Step 1 — Allow PowerShell scripts *(one-time, per user account)*

```powershell
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
```

This allows locally-created scripts (like `.venv\Scripts\Activate.ps1`) to run while still blocking unsigned remote scripts. You only need to do this once; the setting persists across sessions.
...
> ⚠️ **Important:** This project path contains emoji characters (`💼`, `👨‍💻`). The Windows console defaults to cp1252 encoding, which causes a `UnicodeEncodeError` when `pip` tries to print the path. Always set `PYTHONIOENCODING` before activating.

```powershell
$env:PYTHONIOENCODING = "utf-8"; .venv\Scripts\Activate.ps1
```

You will see `(.venv)` appear at the start of your prompt confirming activation.
...
This installs: `Flask`, `Werkzeug`, `click`, `openpyxl`, `pandas`, `numpy`, `gunicorn`, and all their sub-dependencies.

> ⚠️ **Do not skip this step.** A freshly created `.venv` only contains `pip` — nothing else. If you run `python run.py` before installing, you will see:
> ```
> ModuleNotFoundError: No module named 'click'
> ```
> The fix is always the same: run `pip install -r requirements.txt` inside the activated venv.
...
The app starts at **http://127.0.0.1:5000** and automatically redirects to the **ASP Dashboard**.

> **Chrome DevTools probe:** You may see `GET /.well-known/appspecific/com.chrome.devtools.json 404` in the console. This is harmless — Chrome sends it automatically on localhost.

### Every time you return to work

Steps 2 and 4 are one-time setup. On subsequent sessions, just run:
...
| Portal | URL | Status |
|---|---|---|
| ASP Dashboard | `http://127.0.0.1:5000/asp/dashboard` | ✅ Live |
| Work Orders | `http://127.0.0.1:5000/asp/work-orders` | ✅ Live |
| Parts Management | `http://127.0.0.1:5000/asp/parts` | ✅ Live |
| Admin Data Import | `http://127.0.0.1:5000/admin/data-import` | ✅ Live |
| Admin Archive | `http://127.0.0.1:5000/admin/archive` | ✅ Live |
...
**Windows (PowerShell):**
```powershell
# Set UTF-8 encoding first — required when the project path contains emoji characters
$env:PYTHONIOENCODING = "utf-8"; .venv\Scripts\Activate.ps1
```
